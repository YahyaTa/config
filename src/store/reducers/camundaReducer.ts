...
export const camundaGet = createAsyncThunk(
    ...
    async (props: { url: string; header: any; camundaOperation: string; id?: string; subId?: string }) => {
        ...
        if (id) {
            returnGetObj.id = id;
            if (subId) returnGetObj.subId = subId;
        }
...
export const camundaPost = createAsyncThunk(
    ...
    async (props: { url: string; body: any; header: any; camundaOperation: string; id?: string; subId?: string }) => {
        ...
        if (id) {
            returnPostObj.id = id;
            if (subId) returnPostObj.subId = subId;
        }
...
export const camundaPut = createAsyncThunk(
    ...
    async (props: { url: string; body: any; header: any; camundaOperation: string; id?: string; subId?: string }) => {
        ...
        if (id) {
            returnPutObj.id = id;
            if (subId) returnPutObj.subId = subId;
        }
...
export const camundaValidateUnsolPmnt = createAsyncThunk(
    ...
    async (props: { processId: string; contractId: string; documentName: string }, { rejectWithValue }) => {
         ...
         */
        const responseStep1 = await axios.get(
            camundaConfig.defaultCamundaUrl + camundaConfig.requestProcessInstanceActivities.url(processId),
            {
        ...
        let taskId = '';
        if (isStep2_OK || (!isStep1_OK && !isStep2_OK)) {
        ...
        if (isStep4_OK || (!isStep4_OK && !isStep3_OK)) {
            ...
            if (responseStep5.status === 200 && responseStep5.data.state === 'COMPLETED') {
                isStep5_OK = true;
            }
            else {
...
const identicalAddCaseOne = (state: any, action: any) => {
...
const identicalAddCaseTwo = (state: any, action: any) => {
...
const identicalAddCaseThree = (state: any, action: any) => {
    let camundaProperty = getCamundaStateProperty(action.meta.arg.camundaOperation);
...
const camundaSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(camundaGet.pending, (state, action) => identicalAddCaseTwo(state, action));
        builder.addCase(camundaGet.fulfilled, (state, action) => identicalAddCaseOne(state, action));
        builder.addCase(camundaGet.rejected, (state, action) => identicalAddCaseThree(state, action));
        builder.addCase(camundaPost.pending, (state, action) => identicalAddCaseTwo(state, action));
        builder.addCase(camundaPost.fulfilled, (state, action) => identicalAddCaseOne(state, action));
        builder.addCase(camundaPost.rejected, (state, action) => identicalAddCaseThree(state, action));
        builder.addCase(camundaPut.pending, (state, action) => identicalAddCaseTwo(state, action));
        builder.addCase(camundaPut.fulfilled, (state, action) => identicalAddCaseOne(state, action));
        builder.addCase(camundaPut.rejected, (state, action) => identicalAddCaseThree(state, action));
